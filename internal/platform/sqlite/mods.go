...
func scanMod(row scanner) (domain.InstalledMod, error) {
...
func (s *SQLiteStore) GetMod(ctx context.Context, id string) (domain.InstalledMod, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return mod, domain.NewError(domain.ErrModNotFound, "Mod not found")
	}
	return mod, err
}

func (s *SQLiteStore) SaveMod(ctx context.Context, mod domain.InstalledMod) error {
