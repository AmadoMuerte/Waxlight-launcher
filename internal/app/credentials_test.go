...
func TestCredentialStoreUnavailable(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"unlock failed":   {fmt.Errorf("%w: %w: %w", accounts.ErrStoreUnlockFailed, accounts.ErrStoreUnavailable, errors.New("native")), true},
		"desktop session": {fmt.Errorf("%w: %w: %w", accounts.ErrStoreDesktopSession, accounts.ErrStoreUnavailable, errors.New("native")), true},
		"unknown":         {fmt.Errorf("wrapped: %w", accounts.ErrStoreUnknown), true},
